  ...
  viewer.dataSources.add(dataSource).then(res => {
    ...
    handler.setInputAction(function (event) {
      ...
      if (Cesium.defined(earthPosition)) {
        ...
        floatingPoint1 = createPoint(ray_start);
        ...
        var start_to_end = math.distance([ray_start.x, ray_start.y, ray_start.z], [earthPosition.x, earthPosition.y, earthPosition.z])
        if (((hitPos == undefined) && (hitPos == null)) || (start_to_hit > start_to_end)) {
          console.log('losdist',start_to_end)
          ...
          for (let j = 0; j <= pl_set.length - 1; j++) {
            ...
            if ((hitPos_imag_start !== undefined) && (hitPos_imag_start !== null) && (hitPos_imag_end !== undefined) && (hitPos_imag_end !== null)) {
              ...
              if (math.abs(math.subtract(aaax,bbbx)) <= 0.1 && math.abs(math.subtract(aaay,bbby)) <= 0.1 && math.abs(math.subtract(aaaz,bbbz)) <= 0.1) {
                ...
                console.log('firstref',fst_ray_len);
                shape11 = dataSource.entities.add({
                  polyline: {
                    positions: [ray_start, hitPos_imag_end.position],
                    arcType: Cesium.ArcType.NONE,
                    clampToGround: false,
                    width: 3,
                ...
                shape12 = dataSource.entities.add({
                  polyline: {
                    positions: [hitPos_imag_end.position, ray_end],
                    arcType: Cesium.ArcType.NONE,
                    clampToGround: false,
                    width: 3,
                    material: Cesium.Color.YELLOW
